    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{filtered.length} transactions · Revenue: PKR {totalRevenue.toLocaleString()}</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-slate-500 text-xs uppercase tracking-wide">
              {['Invoice #','Cashier','Customer','Items','Total','Payment','Date',''].map(h => (
                <th key={h} className="pb-3 pr-4 font-medium text-left">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {isLoading ? (
              ...
              <tr><td colSpan={8} className="py-12 text-center text-slate-500">No sales found</td></tr>
            ) : filtered.map((s: any) => (
              <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                <td className="py-3 pr-4"><span className="font-mono text-xs text-primary-400 font-semibold">{s.invoiceNumber}</span></td>
                <td className="py-3 pr-4 text-slate-600">{s.cashier?.fullName}</td>
                <td className="py-3 pr-4 text-slate-500">{s.customer?.name || <span className="italic text-slate-600">Walk-in</span>}</td>
                <td className="py-3 pr-4 text-slate-500">{s.items?.length}</td>
                <td className="py-3 pr-4 text-slate-800 font-semibold">PKR {s.total?.toLocaleString()}</td>
                ...
                <td className="py-3">
                  <button onClick={() => setSelectedSale(s)}
                    className="flex items-center gap-1.5 text-xs text-primary-400 hover:text-primary-300 border border-primary-500/30 hover:border-primary-500/60 rounded px-2 py-1 transition-colors">
